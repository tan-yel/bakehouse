...
const yourName = "tan"; // e.g. "sarah", "james", "priya"
const githubOrg = "tan-yel";
const githubRepo = "bakehouse";
const githubRefFilter = "*";
/**
 * Defines the custom settings our Bakehouse stack accepts - without these our stack won't run
 */
...
export class BakehouseStack extends Stack {
  constructor (scope, id, props) {
    ...
    const sharedVpc = ec2.Vpc.fromLookup(this, 'sharedVpc', { // so services can talk to each other without going public
    ...
    const cluster = new rds.DatabaseCluster(this, 'rds-cluster', {
      ...
      vpc: sharedVpc, // cluster will sit in shared VPC and will not be exposed to public internet
      vpcSubnets: {
      ...
      serverlessV2MinCapacity: 0.5, // absolute minimum
      ...
      removalPolicy: cdk.RemovalPolicy.DESTROY // destroy since training env
    })
    ...
    const clientQueryPolicy = cloudfront.OriginRequestPolicy.fromOriginRequestPolicyId(
      ...
      props.sharedOriginRequestPolicyId //default may strip query strings
    )
    ...
    const lambdaEnvVars = {
      NODE_ENV: 'production',
      AWS_NODEJS_CONNECTION_REUSE_ENABLED: '1', // allows multiple connections
      DB_NAME: props.dbName,
      CLUSTER_ARN: cluster.clusterArn,
      SECRET_ARN: cluster.secret?.secretArn || 'NOT_SET', // key to access db
    ...
    const badLambda = new lambda.Function(this, 'bad-lambda', { // changed to lambda.Function since new.NodejsFunction includes bundling
    ...
    const healthcheckLambda = new lambda.Function(this, 'health-check-lambda', {
      ...
      runtime: lambda.Runtime.NODEJS_22_X,
      handler: 'health-check.healthcheckHandler', // what runs everytime our lambda is invoked
      code: lambda.Code.fromAsset('functions'),
      environment: lambdaEnvVars
    })
    ...
    const clientDistribution = new cloudfront.Distribution(this, 'client-distribution', {
      defaultBehavior: {
        origin: new origins.S3BucketOrigin(clientBucket), // will serve the html doc
        viewerProtocolPolicy: cloudfront.ViewerProtocolPolicy.REDIRECT_TO_HTTPS, // will be redirected if they use http
      ...
      additionalBehaviors: {
        '/api/*': { // if request includes /api/* does not need to go to
          origin: new origins.HttpOrigin( // hits the api id instead
            `${api.restApiId}.execute-api.${props.env.region}.amazonaws.com`
          ),
          viewerProtocolPolicy: cloudfront.ViewerProtocolPolicy.REDIRECT_TO_HTTPS,
          allowedMethods: cloudfront.AllowedMethods.ALLOW_ALL,
          cachePolicy: cloudfront.CachePolicy.CACHING_DISABLED // information on sit needs to be up-to-date
        }
      },
      errorResponses: [
        {
          ...
          responsePagePath: '/index.html', // redirect to route of the site
    ...
    new s3Deployment.BucketDeployment(this, 'product-cards-deployment', {
      ...
      sources: [
        s3Deployment.Source.asset(
          path.resolve(__dirname, '../product-cards') // putting the products in the folder into the bucket
        )
      ],
      prune: true,
      memoryLimit: 256,
    ...
    }) // routes the autogenerated name to the alias
    // cnamerecord is an alias is routed too
